...
# ---------------------------
def prepare_client_files(input_file="wikisent2.txt", num_clients=2):
    """
    Split wikisent2.txt into multiple smaller client files
    and save them into clients/ directory.
    """
    ...
    split_size = len(lines) // num_clients

    for i in range(num_clients):
        start = i * split_size
        end = (i + 1) * split_size if i < num_clients - 1 else len(lines)
        client_lines = lines[start:end]

        client_file = f"clients/client{i+1}.txt"
        with open(client_file, "w", encoding="utf-8") as cf:
            cf.write("\n".join(client_lines))

        print(f"✅ Created {client_file} with {len(client_lines)} samples")


def create_client_datasets():
    """Load client datasets from text files in clients/ directory"""
    client_datasets = []
    client_files = sorted([f for f in os.listdir("clients") if f.endswith(".txt")])
...
# ---------------------------
def main():
    # Prepare client data from wikisent2.txt
    prepare_client_files("wikisent2.txt", num_clients=len(Config.DEFAULT_RANKS))
